...
public class CreateTradingZoneValidatorTests
{
    ...
    [Test] public void Should_Not_Have_Error_When_Valid() { var r = _validator.TestValidate(new CreateTradingZone.Request("Asia", "09:30", "16:00", "desc", 1)); r.ShouldNotHaveAnyValidationErrors(); }
}
...
public class GetTradingZoneDetailHandlerTests
{
    ...
    [Test] public async Task Handle_Returns_Failure_When_Not_Found() { _dbMock.Setup(x => x.TradingZones).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradingZoneEntity>().AsQueryable()).Object); var result = await _handler.Handle(new GetTradingZoneDetail.Request(99), CancellationToken.None); Assert.That(result.IsFailure, Is.True); }
    [Test] public async Task Handle_Returns_Success_When_Found() { var zone = new TradingZoneEntity { Id = 1, Name = "Asia", CreatedBy = 1, FromTime = "09:30", ToTime = "16:00" }; _dbMock.Setup(x => x.TradingZones).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradingZoneEntity> { zone }.AsQueryable()).Object); var result = await _handler.Handle(new GetTradingZoneDetail.Request(1), CancellationToken.None); Assert.That(result.IsSuccess, Is.True); Assert.That(result.Value.Name, Is.EqualTo("Asia")); }
}
...
public class UpdateTradingZoneValidatorTests
{
    ...
    [Test] public void Should_Have_Error_When_Name_Is_Empty() { var r = _validator.TestValidate(new UpdateTradingZone.Request(1, "", "09:30", "16:00", "desc", 1)); r.ShouldHaveValidationErrorFor(x => x.Name); }
    [Test] public void Should_Not_Have_Error_When_Valid() { var r = _validator.TestValidate(new UpdateTradingZone.Request(1, "Asia", "09:30", "16:00", "desc", 1)); r.ShouldNotHaveAnyValidationErrors(); }
}
...
public class DeleteTradingZoneValidatorTests
{
    ...
    [Test] public void Should_Not_Have_Error_When_Valid() { var r = _validator.TestValidate(new DeleteTradingZone.Request(1, 1)); r.ShouldNotHaveAnyValidationErrors(); }
}
