...
public class EndTradeSessionHandlerTests
{
    ...
    [Fact] public async Task Handle_Returns_Failure_When_Not_Found() { _dbMock.Setup(x => x.TradingSessions).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradingSessionEntity>().AsQueryable()).Object); _dbMock.Setup(x => x.TradeHistories).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradeHistory>().AsQueryable()).Object); var result = await _handler.Handle(new EndTradeSession.Request(99, DateTime.UtcNow, null, null, 1), CancellationToken.None); Assert.True(result.IsFailure); }
    [Fact] public async Task Handle_Returns_Failure_When_Session_Belongs_To_Another_User() { var session = new TradingSessionEntity { Id = 1, FromTime = DateTime.UtcNow, Status = TradingJournal.Modules.Trades.Common.Enum.TradingSessionStatus.Active, CreatedBy = 7 }; _dbMock.Setup(x => x.TradingSessions).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradingSessionEntity> { session }.AsQueryable()).Object); _dbMock.Setup(x => x.TradeHistories).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradeHistory>().AsQueryable()).Object); var result = await _handler.Handle(new EndTradeSession.Request(1, DateTime.UtcNow, null, null, 1), CancellationToken.None); Assert.True(result.IsFailure); _dbMock.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Never); }
    [Fact] public async Task Handle_Returns_Success_When_Ended() { var session = new TradingSessionEntity { Id = 1, FromTime = DateTime.UtcNow, Status = TradingJournal.Modules.Trades.Common.Enum.TradingSessionStatus.Active, CreatedBy = 1 }; _dbMock.Setup(x => x.TradingSessions).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradingSessionEntity> { session }.AsQueryable()).Object); _dbMock.Setup(x => x.TradeHistories).Returns(DbSetMockHelper.CreateMockDbSet(new List<TradeHistory>().AsQueryable()).Object); _dbMock.Setup(x => x.SaveChangesAsync(It.IsAny<CancellationToken>())).ReturnsAsync(1); var result = await _handler.Handle(new EndTradeSession.Request(1, DateTime.UtcNow, null, null, 1), CancellationToken.None); Assert.True(result.IsSuccess); Assert.Equal(TradingJournal.Modules.Trades.Common.Enum.TradingSessionStatus.Closed, session.Status); }
}
