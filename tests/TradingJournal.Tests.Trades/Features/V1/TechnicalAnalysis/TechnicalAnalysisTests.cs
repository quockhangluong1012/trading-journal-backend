...
public class GetTechnicalAnalysisDetailHandlerTests
{
    ...
    [Fact] public async Task Handle_Returns_Failure_When_Not_Found() { _dbMock.Setup(x => x.TechnicalAnalyses).Returns(DbSetMockHelper.CreateMockDbSet(new List<TechnicalAnalysisEntity>().AsQueryable()).Object); var result = await _handler.Handle(new GetTechnicalAnalysisDetail.Request(99, 1), CancellationToken.None); Assert.True(result.IsFailure); }
    [Fact] public async Task Handle_Returns_Failure_When_Analysis_Belongs_To_Another_User() { var ta = new TechnicalAnalysisEntity { Id = 1, Name = "SMA", CreatedBy = 7, ShortName = "SMA", Description = "desc" }; _dbMock.Setup(x => x.TechnicalAnalyses).Returns(DbSetMockHelper.CreateMockDbSet(new List<TechnicalAnalysisEntity> { ta }.AsQueryable()).Object); var result = await _handler.Handle(new GetTechnicalAnalysisDetail.Request(1, 1), CancellationToken.None); Assert.True(result.IsFailure); }
    [Fact] public async Task Handle_Returns_Success_When_Found() { var ta = new TechnicalAnalysisEntity { Id = 1, Name = "SMA", CreatedBy = 1, ShortName = "SMA", Description = "desc" }; _dbMock.Setup(x => x.TechnicalAnalyses).Returns(DbSetMockHelper.CreateMockDbSet(new List<TechnicalAnalysisEntity> { ta }.AsQueryable()).Object); var result = await _handler.Handle(new GetTechnicalAnalysisDetail.Request(1, 1), CancellationToken.None); Assert.True(result.IsSuccess); Assert.Equal("SMA", result.Value.Name); }
}
