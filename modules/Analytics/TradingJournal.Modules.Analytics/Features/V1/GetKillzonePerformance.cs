using TradingJournal.Shared.Common.Enum;

namespace TradingJournal.Modules.Analytics.Features.V1;

public sealed class GetKillzonePerformance
{
    internal sealed record Request(AnalyticsFilter Filter, int UserId = 0)
        : IQuery<Result<KillzonePerformanceResponse>>;

    internal sealed record KillzonePerformanceViewModel(
        int ZoneId,
        string ZoneName,
        string TimeWindow,
        int TotalTrades,
        int Wins,
        int Losses,
        decimal WinRate,
        decimal TotalPnl,
        decimal AvgPnl,
        decimal AvgWin,
        decimal AvgLoss,
        decimal ProfitFactor,
        decimal Expectancy,
        decimal AvgRiskReward,
        string BestDay,
        string Grade);

    internal sealed record DayZoneHeatmapCell(
        string Day,
        string Zone,
        int Count,
        decimal Pnl,
        decimal WinRate);

    internal sealed record KillzonePerformanceResponse(
        IReadOnlyCollection<KillzonePerformanceViewModel> Zones,
        IReadOnlyCollection<DayZoneHeatmapCell> Heatmap);

    internal sealed class Validator : AbstractValidator<Request>
    {
        public Validator()
        {
            RuleFor(x => x.Filter)
                .IsInEnum()
                .WithErrorCode(HttpStatusCode.BadRequest.ToString())
                .WithMessage("Invalid filter value.");
        }
    }

    internal sealed class Handler(
        ITradeProvider tradeProvider,
        IZoneProvider zoneProvider) : IQueryHandler<Request, Result<KillzonePerformanceResponse>>
    {
        public async Task<Result<KillzonePerformanceResponse>> Handle(
            Request request, CancellationToken cancellationToken)
        {
            List<TradeCacheDto> trades = await tradeProvider.GetTradesAsync(request.UserId, cancellationToken);
            List<ZoneSummaryDto> zones = await zoneProvider.GetZonesAsync(cancellationToken);
            DateTimeOffset fromDate = AnalyticsFilterHelper.GetFromDate(request.Filter);

            // Filter to closed trades with a zone assigned
            List<TradeCacheDto> closed = [.. trades
                .Where(t => t.Status == TradeStatus.Closed && t.Pnl.HasValue && t.TradingZoneId.HasValue)
                .Where(t => fromDate == DateTimeOffset.MinValue || (t.ClosedDate.HasValue && t.ClosedDate.Value >= fromDate))];

            // Build zone name lookup
            Dictionary<int, ZoneSummaryDto> zoneLookup = zones.ToDictionary(z => z.Id);

            string[] dayNames = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

            // Group by zone and calculate metrics
            List<KillzonePerformanceViewModel> zoneMetrics = [.. closed
                .GroupBy(t => t.TradingZoneId!.Value)
                .Select(g =>
                {
                    List<TradeCacheDto> groupTrades = [.. g];
                    List<TradeCacheDto> wins = groupTrades.Where(t => t.Pnl > 0).ToList();
                    List<TradeCacheDto> losses = groupTrades.Where(t => t.Pnl <= 0).ToList();

                    decimal totalPnl = groupTrades.Sum(t => t.Pnl!.Value);
                    decimal winRate = (decimal)wins.Count / groupTrades.Count * 100;
                    decimal avgPnl = groupTrades.Average(t => t.Pnl!.Value);
                    decimal avgWin = wins.Count > 0 ? wins.Average(t => t.Pnl!.Value) : 0;
                    decimal avgLoss = losses.Count > 0 ? Math.Abs(losses.Average(t => t.Pnl!.Value)) : 0;

                    decimal grossProfit = wins.Sum(t => t.Pnl!.Value);
                    decimal grossLoss = Math.Abs(losses.Sum(t => t.Pnl!.Value));
                    decimal profitFactor = grossLoss > 0
                        ? grossProfit / grossLoss
                        : (grossProfit > 0 ? decimal.MaxValue : 0);

                    decimal expectancy = (winRate / 100 * avgWin) - ((1 - winRate / 100) * avgLoss);

                    // Avg risk-reward
                    double[] rrValues = groupTrades
                        .Where(t => t.StopLoss > 0 && t.TargetTier1 > 0 && t.EntryPrice > 0)
                        .Select(t =>
                        {
                            decimal risk = Math.Abs(t.EntryPrice - t.StopLoss);
                            decimal reward = Math.Abs(t.TargetTier1 - t.EntryPrice);
                            return risk > 0 ? (double)(reward / risk) : 0;
                        })
                        .Where(r => r > 0)
                        .ToArray();
                    decimal avgRiskReward = rrValues.Length > 0 ? (decimal)rrValues.Average() : 0;

                    // Best day of week for this zone
                    string bestDay = groupTrades
                        .Where(t => t.ClosedDate.HasValue)
                        .GroupBy(t => t.ClosedDate!.Value.DayOfWeek)
                        .Select(dg => new
                        {
                            Day = dayNames[(int)dg.Key],
                            Pnl = dg.Sum(t => t.Pnl!.Value)
                        })
                        .OrderByDescending(x => x.Pnl)
                        .FirstOrDefault()?.Day ?? "N/A";

                    // Zone info
                    ZoneSummaryDto? zone = zoneLookup.GetValueOrDefault(g.Key);
                    string zoneName = zone?.Name ?? $"Zone #{g.Key}";
                    string timeWindow = zone is not null ? $"{zone.FromTime} - {zone.ToTime}" : "";

                    string grade = CalculateGrade(winRate, profitFactor, groupTrades.Count);

                    return new KillzonePerformanceViewModel(
                        g.Key,
                        zoneName,
                        timeWindow,
                        groupTrades.Count,
                        wins.Count,
                        losses.Count,
                        Math.Round(winRate, 1),
                        Math.Round(totalPnl, 2),
                        Math.Round(avgPnl, 2),
                        Math.Round(avgWin, 2),
                        Math.Round(avgLoss, 2),
                        Math.Round(profitFactor, 2),
                        Math.Round(expectancy, 2),
                        Math.Round(avgRiskReward, 2),
                        bestDay,
                        grade);
                })
                .OrderByDescending(z => z.TotalPnl)];

            // Build heatmap: Day × Zone matrix
            List<DayZoneHeatmapCell> heatmap = [.. closed
                .Where(t => t.ClosedDate.HasValue)
                .GroupBy(t => new { Day = t.ClosedDate!.Value.DayOfWeek, ZoneId = t.TradingZoneId!.Value })
                .Select(g =>
                {
                    string zoneName = zoneLookup.GetValueOrDefault(g.Key.ZoneId)?.Name ?? $"Zone #{g.Key.ZoneId}";
                    int count = g.Count();
                    decimal pnl = g.Sum(t => t.Pnl!.Value);
                    decimal winRate = count > 0 ? (decimal)g.Count(t => t.Pnl > 0) / count * 100 : 0;

                    return new DayZoneHeatmapCell(
                        dayNames[(int)g.Key.Day],
                        zoneName,
                        count,
                        Math.Round(pnl, 2),
                        Math.Round(winRate, 1));
                })
                .OrderBy(h => Array.IndexOf(dayNames, h.Day))];

            return Result<KillzonePerformanceResponse>.Success(
                new KillzonePerformanceResponse(zoneMetrics, heatmap));
        }

        private static string CalculateGrade(decimal winRate, decimal profitFactor, int totalTrades)
        {
            if (totalTrades < 5)
                return "N/A";

            if (winRate >= 65 && profitFactor >= 2)
                return "A";
            if (winRate >= 55 && profitFactor >= 1.5m)
                return "B";
            if (winRate >= 45 && profitFactor >= 1)
                return "C";
            if (winRate >= 35)
                return "D";

            return "F";
        }
    }

    public sealed class Endpoint : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            RouteGroupBuilder group = app.MapGroup("api/v1/analytics");

            group.MapGet("/killzone-performance", async (AnalyticsFilter filter, ClaimsPrincipal user, ISender sender) =>
            {
                var result = await sender.Send(new Request(filter) with { UserId = user.GetCurrentUserId() });
                return result.IsSuccess ? Results.Ok(result) : Results.Problem(result.Errors[0].Description);
            })
            .Produces<Result<KillzonePerformanceResponse>>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status400BadRequest)
            .WithSummary("Get killzone performance breakdown.")
            .WithDescription("Retrieves performance metrics grouped by trading zone/killzone, including a day×zone heatmap for identifying the best time windows.")
            .WithTags(Tags.Analytics)
            .RequireAuthorization();
        }
    }
}
