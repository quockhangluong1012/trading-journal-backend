...
public sealed class ExportReviewReport
{
    ...
    public sealed class Handler(IReviewSnapshotBuilder reviewSnapshotBuilder) : IQueryHandler<Request, Result<ReviewReportFile>>
    {
        ...
        private static string BuildCsv(ReviewSnapshot snapshot)
        {
            ...
            if (!string.IsNullOrWhiteSpace(metrics.TopAsset))
                sb.AppendLine($"Top Asset,{metrics.TopAsset}");

            if (!string.IsNullOrWhiteSpace(metrics.PrimaryTradingZone))
                sb.AppendLine($"Primary Trading Zone,{metrics.PrimaryTradingZone}");

            if (!string.IsNullOrWhiteSpace(metrics.DominantEmotion))
                sb.AppendLine($"Dominant Emotion,{metrics.DominantEmotion}");

            if (!string.IsNullOrWhiteSpace(metrics.TopTechnicalTheme))
                sb.AppendLine($"Top Technical Theme,{metrics.TopTechnicalTheme}");
            ...
            foreach (ReviewTradeInsight trade in snapshot.Trades)
            {
                sb.AppendLine(string.Join(",", [
                    ...
                    trade.ConfidenceLevel.ToString(),
                    EscapeCsvField(trade.TradingZone ?? ""),
                    trade.IsRuleBroken ? "Yes" : "No",
                    EscapeCsvField(trade.RuleBreakReason ?? ""),
                    EscapeCsvField(string.Join("; ", trade.EmotionTags)),
                    EscapeCsvField(string.Join("; ", trade.TechnicalThemes)),
                    EscapeCsvField(string.Join("; ", trade.ChecklistItems)),
            ...
            {
                sb.AppendLine();
                sb.AppendLine("=== PSYCHOLOGY NOTES ===");
                sb.AppendLine();

                foreach (string note in snapshot.PsychologyNotes)
                {
                    sb.AppendLine(EscapeCsvField(note));
                }
            }
    ...
    public sealed class Endpoint : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            ...
            group.MapGet("/export", async (ReviewPeriodType periodType, DateTime periodStart, ClaimsPrincipal user, ISender sender) =>
            {
                ...
                if (!result.IsSuccess)
                {
                    return Results.Problem(result.Errors[0].Description);
                }
