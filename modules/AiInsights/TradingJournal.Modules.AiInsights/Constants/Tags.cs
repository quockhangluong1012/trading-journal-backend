namespace TradingJournal.Modules.AiInsights.Constants;

public static class Tags
{
    public const string Reviews = "Reviews";

    public const string AiCoach = "AI Coach";

    public const string AiValidation = "AI Validation";

    public const string AiEmotions = "AI Emotions";

    public const string AiBriefing = "AI Briefing";

    public const string AiSearch = "AI Search";

    public const string AiPatterns = "AI Patterns";

    public const string AiLessons = "AI Lessons";

    public const string AiPlaybook = "AI Playbook";
}

public static class ApiGroup
{
    public static class V1
    {
        internal const string Reviews = "api/v1/reviews";

        internal const string AiCoach = "api/v1/ai-coach";

        internal const string AiValidation = "api/v1/ai-validation";

        internal const string AiEmotions = "api/v1/ai-emotions";

        internal const string AiBriefing = "api/v1/ai-briefing";

        internal const string AiSearch = "api/v1/ai-search";

        internal const string AiPatterns = "api/v1/ai-patterns";

        internal const string AiLessons = "api/v1/ai-lessons";

        internal const string AiPlaybook = "api/v1/ai-playbook";
    }
}
