...
public sealed class GetReview
{
    ...
    public sealed record ReviewViewModel(
        ...
        string? AiWhatToImprove, bool AiSummaryGenerating,
        decimal TotalPnl, decimal WinRate, int TotalTrades, int Wins, int Losses,
        decimal AverageWin, decimal AverageLoss, decimal BestTradePnl, decimal WorstTradePnl,
        decimal BestDayPnl, decimal WorstDayPnl, int LongTrades, int ShortTrades,
        int RuleBreakTrades, int HighConfidenceTrades,
        string? TopAsset, string? PrimaryTradingZone, string? DominantEmotion, string? TopTechnicalTheme);
    ...
    public sealed class Handler(IAiInsightsDbContext context, IAiTradeDataProvider tradeDataProvider)
        : IQueryHandler<Request, Result<ReviewViewModel>>
    {
        ...
        private static ReviewViewModel ToViewModel(TradingReview? review, ReviewSnapshot snapshot)
        {
            ...
            return new ReviewViewModel(
                review?.Id, snapshot.PeriodType, snapshot.PeriodStart, snapshot.PeriodEnd,
                review?.UserNotes, review?.AiSummary, review?.AiStrengths, review?.AiWeaknesses,
                ...
                review?.AiWhatToImprove, review?.AiSummaryGenerating ?? false,
                m.TotalPnl, m.WinRate, m.TotalTrades, m.Wins, m.Losses,
                m.AverageWin, m.AverageLoss, m.BestTradePnl, m.WorstTradePnl,
                m.BestDayPnl, m.WorstDayPnl, m.LongTrades, m.ShortTrades,
                m.RuleBreakTrades, m.HighConfidenceTrades,
                m.TopAsset, m.PrimaryTradingZone, m.DominantEmotion, m.TopTechnicalTheme);
        }
    }
    ...
    public sealed class Endpoint : ICarterModule
    {
        public void AddRoutes(IEndpointRouteBuilder app)
        {
            ...
            .Produces<Result<ReviewViewModel>>().Produces(StatusCodes.Status400BadRequest)
